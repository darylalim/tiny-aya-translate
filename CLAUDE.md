...
- Language selectboxes use the flat `LANGUAGES` list (67 items) with collapsed labels and Streamlit's built-in type-to-search; each tab's language bar is wrapped in `st.container(border=True)` to group the from/swap/to row as a card
...
- Both theme variants set `primaryColor = "#5e81ac"` (deep frost, not the lighter `#88c0d0`): Streamlit renders primary-button labels white, so the primary must clear WCAG contrast. `test_primary_button_text_readable_in_both_modes` computes the white-on-primary ratio (≥ 3:1) and guards it
